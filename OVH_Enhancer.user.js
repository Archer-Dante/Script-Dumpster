...
// @author    Archer-Dante
// @updateURL https://github.com/Haibane-Kira/Script-Dumpster/blob/master/OVH_Enhancer.user.js
// ==/UserScript==
...
//if ((location.href).indexOf("steamkeys.ovh") > 0) {
    ...
    var temp_rate = 16,
        ...
        deals_left = deals_info.substr(deals_pos1, deals_pos2);
            $('#buttons center').append('<div class="btn" id="blank_button" style="opacity:0">Что ищем?</div>');
      ...
      $('#buttons center').append('<div class="btn" id="hks_forms" style="background:#9e1f1f;width:130px;font-size:0.9em;cursor:default">Рейт продажи сета<br><input id="sets_rate" type="text" size="14" placeholder="Рейт продажи сета" value="' + temp_rate + '"></div>')
       $('#buttons center').append('<div class="btn" id="hks_forms" style="background:#9e1f1f;width:130px;font-size:0.9em;cursor:default">Цена продажи ключа<br><input id="sell_key_price" type="text" size="14" placeholder="Цена продажи ключа" value="' + temp_kSell + '"></div>')
        ...
        $('#buttons center').append('<div class="btn" id="hks_forms" style="background:#9e1f1f;width:130px;font-size:0.9em;cursor:default">Позиций<br><input id="calc_deals" type="text" size="14" placeholder="Позиций" value="' + deals_left + '"></div>');
          $('#buttons center').append('<div class="btn" id="hks_forms" style="background:#9e1f1f;width:130px;font-size:0.9em;cursor:default">Мин. профит %<br><input id="min_profit" type="text" size="14" placeholder="Мин. профит %" value="' + temp_minprofit + '"></div>');
          ...
          var   sell_key_price = $('#sell_key_price').val(),
           ...
           i++;     
                ...
                var   price = $('#DataTables_Table_0 tbody > tr:nth-child(' + i + ') > td:nth-child(4) a').html(),
                    ...
                    set_val = Number(title.substr(set_pos1, set_pos2));
                     ...
                     var set_size = $('tbody:not(:nth-child(1)) > tr:nth-child(' + i + ') > td:nth-child(1) a').html(),
                                      ...
                                      prelast_counter_MARKET = ((Math.ceil(set_size / 2) * bot * (set_val / set_size)) / 100 * part_of_cake / 100 * steam_to_qiwi),
                                      profit_MARKET = (Math.ceil(prelast_counter_MARKET / cost * 100 - 100)).toFixed(0),
                                      prelast_counter_SETSELL = (Math.ceil(set_size / 2) * bot / set_size / sets_rate * sell_key_price),
                                      profit_SETSELL = (Math.ceil(prelast_counter_SETSELL / cost * 100 - 100)).toFixed(0);

                         min_money_from_MARKET = (prelast_counter_MARKET - cost).toFixed(2),
                         set_value_for_qiwi_MARKET = (set_val / 100 * part_of_cake / 100 * steam_to_qiwi).toFixed(2),
                         min_money_from_SETSELL = (prelast_counter_SETSELL - cost).toFixed(2),
                ...
                if (Number(min_money_from_MARKET) <= 0 & Number(min_money_from_SETSELL) <= 0) {
                  ...
                  } else if (Number(profit_MARKET) < min_profit & Number(profit_SETSELL) < min_profit) {
                  ...
                  //    $('#DataTables_Table_0 tbody > tr:nth-child(' + i + ')').css('background-color','#efefef'); // #fec
                  } else {
                  ...
                  if (Number(min_money_from_MARKET) > Number(min_money_from_SETSELL)) {
                      $('#DataTables_Table_0 tbody > tr:nth-child(' + i + ') > td:nth-child(9)').html("<center><b>" + min_money_from_MARKET + " р.</b><br><span style='font-size:0.8em'>Профит:" + profit_MARKET +"%</span></center>").css("line-height","90%");
                      $('#DataTables_Table_0 tbody > tr:nth-child(' + i + ') > td:nth-child(10)').html("<center><span style='color:#bbb'>" + min_money_from_SETSELL + " р.</span><br><span style='font-size:0.8em'>Профит:" + profit_SETSELL +"%</span></center>").css("line-height","90%");
                  } else {
                      $('#DataTables_Table_0 tbody > tr:nth-child(' + i + ') > td:nth-child(9)').html("<center><span style='color:#bbb'>" + min_money_from_MARKET + " р.</span><br><span style='font-size:0.8em'>Профит:" + profit_MARKET +"%</span></center>").css("line-height","90%");
                      $('#DataTables_Table_0 tbody > tr:nth-child(' + i + ') > td:nth-child(10)').html("<center><b>" + min_money_from_SETSELL + " р.</b><br><span style='font-size:0.8em'>Профит:" + profit_SETSELL +"%</span></center>").css("line-height","90%");
                  }
